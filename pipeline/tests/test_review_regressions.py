...
def test_harvest_persists_observation_date(tmp_path, monkeypatch):
    ...
    entry = next(iter(json.loads(cache.read_text(encoding="utf-8")).values()))
...
CASES = json.loads((Path(__file__).parents[2] / 'shared/filter-cases.json').read_text(encoding="utf-8"))
...
def test_mobile_off_dictionary_matches_pipeline():
    ...
    assert json.loads(path.read_text(encoding="utf-8")) == _TAG_TO_ALLERGEN
