...
_NON_FOOD_KEYWORDS: tuple[str, ...] = (
    ...
    "ליבידרם",
    "קולגייט",
    # כלי בית, חשמל וטקסטיל. רשתות המזון מוכרות גם אותם, וכל עוד הם
    # אינם מסווגים הם נספרים כמוצרי מזון שאין עליהם מידע אלרגנים.
    #
    # כל מונח כאן נבחר בתנאי אחד: שאינו יכול להופיע בשם של מזון. זה
    # אינו קפדנות יתר אלא הכיוון המסוכן של הכלל הזה — סימון מזון
    # כלא-מזון מסתיר את טבלת האלרגנים שלו לגמרי. לכן "קרם" ו"שמן"
    # אינם כאן, ו"מזוודה" ו"שטיח" כן.
    # חמישה מונחים נוסו כאן ונפסלו, וכל אחד מהם היה מסתיר אלרגנים של
    # מוצר מזון אמיתי שנמצא בקטלוג:
    #   "אוזניות" — "עוגיות אוזניות" היא צורת עוגייה.
    #   "שטיחים"  — "לוקיטוס שטיחים חמוצי" הוא סוכריה.
    #   "מקרר"    — "עוגיות אלפחורס מקרר" פירושו מצונן.
    #   "כרית"    — "כריות" הן דגני בוקר.
    #   "מיקרוגל" — מופיע על פופקורן ועל תפוחי אדמה.
    # מכאן הכלל: מונח נכנס רק אם הוא אינו יכול להופיע בשם של מזון, ואת
    # זה בודקים מול הקטלוג ולא מול האינטואיציה.
    "מזוודה",
    "מזוודות",
    "מקפיא",
    "טלפון",
    ...
    "אקססוריז",
    "מטען",
    # "אלבד" הוא בד לא ארוג, ונבחר במקום "מפה" שיכולה להיות גם מפת
    # שולחן וגם מילה בשם של מוצר אחר.
    "אלבד",
    "מפת שולחן",
    # טואלטיקה וכלי בית שנמדדו מול הקטלוג. חלקם מתקנים גם שיוך שגוי
    # קיים: "ת.רחצה מלפפון" ישב ב"פירות וירקות" בגלל המלפפון, ו"קלמר
    # גלידה" ישב בקינוחים.
    #
    # "ספריי" ו"כוס" נבדקו ונפסלו: "אושן ספריי חמוציות" הוא משקה,
    # ו"נמס בכוס פטריות" הוא מרק. "תיק" נפסל כי הוא כללי מדי.
    "רחצה",
    "לשיער",
    "צבע לשיער",
    "מסיר איפור",
...
_RULES: tuple[_Rule, ...] = (
    # מוצרי המדף הייעודי קודמים לסוג המזון, וזו החלטה ולא סדר מקרי.
    #
    # הכלל היה אחרון, ולכן "לחם לבן ללא גלוטן" נתפס קודם ככלל הלחם והגיע
    # למחלקת הלחמים. מבחינת סוג המזון זה נכון, ומבחינת מי שמחפש אותו זה
    # חסר תועלת: אדם שמסנן גלוטן מחפש את המדף הייעודי, לא את מדף הלחם.
    # מתוך 65 מוצרים ששמם "ללא גלוטן" רק 21 הגיעו למחלקה הזו.
    #
    # הסיווג הוא נראות בלבד ואינו קביעת בטיחות. הוא אינו מסמן דבר בירוק
    # ואינו מייצר קביעת היעדר; ADR-0008 קובע ששם קמעונאי אינו ראיית
    # אריזה. המוצר נמצא, והתשובה על האלרגנים נשארת מה שהמקורות אומרים.
    _Rule("health_free_from", ("ללא גלוטן", "נטול גלוטן", "ללא לקטוז", "נטול לקטוז")),
